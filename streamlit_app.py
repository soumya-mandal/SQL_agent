...
def create_rag_index(rag_data_path: str = "rag_db.pkl"):
    ...
    rag_data = [
        ...
        ("What is the average revenue per transaction handled by each staff member?", "SELECT s.staff_id, s.first_name, s.last_name, COUNT(DISTINCT p.payment_id) AS total_transactions, SUM(p.amount) / COUNT(DISTINCT p.payment_id) AS avg_revenue_per_transaction FROM payment p JOIN staff s ON p.staff_id = s.staff_id GROUP BY s.staff_id;"),
        ("Rank customers based on their monthly revenue contribution.", "SELECT customer_id, month, total, RANK() OVER (PARTITION BY month ORDER BY total DESC) AS rank FROM (SELECT c.customer_id, DATE_FORMAT(p.payment_date, '%Y-%m') AS month, SUM(p.amount) AS total FROM payment p JOIN customer c ON p.customer_id = c.customer_id GROUP BY c.customer_id, month) AS monthly_revenue;"),
        ("What is the average revenue per customer in each city?", "SELECT ci.city, AVG(total) AS avg_revenue_per_customer FROM (SELECT c.customer_id, ci.city, SUM(p.amount) AS total FROM payment p JOIN customer c ON p.customer_id = c.customer_id JOIN address a ON c.address_id = a.address_id JOIN city ci ON a.city_id = ci.city_id GROUP BY c.customer_id, ci.city) AS city_sales GROUP BY ci.city ORDER BY avg_revenue_per_customer DESC;"),
