...
def create_rag_index(rag_data_path: str = "rag_db.pkl"):
    ...
    rag_data = [
        ...
        ("Who are the top 5 customers based on the amount they’ve spent?", "SELECT c.customer_id, c.first_name, c.last_name, SUM(p.amount) AS total_spent FROM payment p JOIN customer c ON p.customer_id = c.customer_id GROUP BY c.customer_id, c.first_name, c.last_name ORDER BY total_spent DESC LIMIT 5;"),
